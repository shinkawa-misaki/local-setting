namespace UI
# 端末が16色以上をサポートしていて、かつTTYかどうか」を判定する
# これが真ならば、以降に本格的なANSIエスケープシーケンスで色を出力するエイリアスを定義
alias UI.Color.IsAvailable='[ $(tput colors 2> /dev/null || echo 0) -ge 16 ] && [ -t 1 ]'
if UI.Color.IsAvailable
then

  # もし色非対応の端末なら、すべてのエイリアスがただの echo にフォールバック
...
# 色表示可能かつUnicode使用許可かつ実際に絵文字が出るかをテスト
# これが真ならば、UI.Powerline.Star や UI.Powerline.OK などのエイリアスが、★や✔といったUnicodeシンボルを出力
...
# ========================================================
# 引数で数値コードを受け取り、その場でANSIシーケンスを生成する関数
# ========================================================
