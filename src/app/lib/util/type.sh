...
#===========================================================================================
# プリミティブ値に extension fingerprint が付与されている場合、対象 fingerprint を持つ拡張型名を返す?
# ==========================================================================================
...
#=====================================================
#変数をメソッドチェーン風に扱う?
#暗黙的に Type::Handle へ委譲するシェル関数を動的生成してる？
#=====================================================
...
　# すでに @resolve:this もしくは同義の __local_return_self_and_result=false が含まれているなら 何もしない。未挿入の場合のみ続行。
 ...
 # メソッド本文に @return マクロが無い場合
 # @return “直前” の位置マーカーを使い、@resolve:this をピンポイントで差し込む。。。
...
Type::ConvertAllOfTypeToMethodsIfNeeded() {
  ...
  # 取得したすべてのメソッドに対してType::InjectThisResolutionIfNeeded を順番に呼び出し、
  # メソッド内に @resolve:this が漏れていないか をチェック＆自動挿入
...
Type::InitializePrimitive() {
  ...
  # 先ほどの関数を呼び出し、 その型に紐づくすべてのメソッド へ @resolve:this を自動挿入
...
Type::TrapAndCreate() {
  ...
  # 配列の先頭要素より、“どのコマンドか” を抽出。declare か string か array か??
  local command="${commandWithArgs[0]}"

  # 配列の先頭（コマンド名）を取り除き、残りの引数部分 を次の処理へ渡す準備を整える。
  shift
...
　# 特定コマンドの無視
  ...
  local varName="${varDeclaration%%=*}"　　# 変数名を抽出

  # var value is only important if making an object later on from it
  local varValue="${varDeclaration#*=}"  # 変数名を抽出

  # TODO: make this better, otherwise edge case bug:
  if [[ "$varValue" == "$varName" ]]  # 不要なバグ防止措置?
  then
    # Log "equal $varName=$varValue"
    local varValue=""
  fi

  if [[ ! -z $__typeCreate_varType ]]  # 型宣言コンテキストの確認
  then
